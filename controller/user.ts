...
import { ListParamsInterface, InterfaceFindAllObject } from '../global.interface';



function sortMenus(authMenus: AppRouterItemInterface[], localMenus: AppRouterItemInterface[]) {
    ...
    if (userData != null) {
        ...
        ctx.rest({
            token: jwt.sign({ data: uData }, config.jwt.secret, {
                expiresIn: remember ? config.jwt.maxExpiresIn : config.jwt.expiresIn
            }),
            user: result
        })
    }
 ...
 * @param {Object} ctx
 */
async function registerUser(params: UserCreationAttributes, isCode = false, ctx: Context): Promise<User> {
    if (!params.username) {
    ...
    if (!params.password || params.password.length < 6 || params.password.length > 18 ) {
...
export const getTokenUser = async (ctx: Context, next: () => Promise<void>) => {
    ...
    let menus = null
    let authority: AuthorityInterface = await userApi.getUserAuth(userInfo.data.id)
...
export const getUserList = async (ctx: Context, next: () => Promise<void>) => {
    ...
    let result: InterfaceFindAllObject = await userApi.getUserList(params as ListParamsInterface)
    ctx.rest({
        list: result.rows,
        count: result.count
    })
...
export const getSvgCaptcha = async (ctx: Context, next: () => Promise<void>) => {
...
export const getMyUserInfo = async (ctx: Context, next: () => Promise<void>) => {
