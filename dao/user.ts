...
// 获取用户信息(不包含密码)
export const getUser = async (where: object): Promise<User | null> => {
...
// 获取用户信息(全部信息)
export const getUserAllInfo = async (where: object): Promise<User | null> => {
    let user: User | null = await models.User.findOne({
        where,
    })
    return user;
...
export const addUser = async (
    ...
    let user: User = await models.User.create(
        userAttr
    );
    return user;
}
...
// 给指定用户添加角色
export const addUserRole = async (userId: string | number, roleIds: Array<number|string>): Promise<void> => {
...
// 修改用户角色
export const updateUserRole = async (userId: string | number, roleIds: Array<number>): Promise<void> => {
    let user: User | null = await models.User.findByPk(userId);
    if (user != null) {
...
// 修改用户信息
export const updateUser = async (id: number | string, user: any): Promise<void> => {
...
// 获取用户信息和对应角色信息
export const getUserRole = async (userId: string | number): Promise<User | null> => {
...
// 获取用户列表(包含拥有角色）
export const getUserList = async (listParams: ListParamsInterface): Promise<InterfaceFindAllObject> => {
    let pageNumber: number = listParams.pageNumber ? Number(listParams.pageNumber) - 1 : 0;
...
export const getAuthByUserId = async (userId: number | string): Promise<Role[]> => {
    let roles: Role[] = await models.Role.findAll({
        include: [{
            model: models.User,
            as: 'users',
